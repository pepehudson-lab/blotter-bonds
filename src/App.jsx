                ...
                {filtradas.map((t) => (
                  ...
                  <tr className="fila" style={{ cursor: "pointer", background: seleccionadas.has(t.id) ? "#fff8ec" : undefined }} onClick={() => setFilaExp(filaExp === t.id ? null : t.id)}>
                    ...
                    <td className="td" style={{ textAlign: "right" }}>
                      {t.compradores?.length
                        ? t.compradores.map((r,i) => <div key={i} style={{ fontSize: 10, color: "#1a7a3a", fontWeight: 700 }}>{Number(r.titulos).toLocaleString("es-MX")}</div>)
                        : <span style={{ fontSize: 10, color: "#8a7050" }}>{Number(t.titulos).toLocaleString("es-MX")}</span>
                      }
                    </td>
                    <td className="td" style={{ textAlign: "right", color: "#1a7a3a" }}>
                      {t.compradores?.length ? t.compradores.map((r,i)=><div key={i} style={{fontSize:10,fontWeight:800}}>{fmt(r.px,4)}</div>) : <span style={{fontSize:10,fontWeight:800}}>{fmt(t.pxCompra,4)}</span>}
                    </td>
                    ...
                    <td className="td" style={{ textAlign: "right" }}>
                      {t.vendedores?.length
                        ? t.vendedores.map((r,i) => <div key={i} style={{ fontSize: 10, color: "#c02020", fontWeight: 700 }}>{Number(r.titulos).toLocaleString("es-MX")}</div>)
                        : <span style={{ fontSize: 10, color: "#8a7050" }}>{Number(t.titulos).toLocaleString("es-MX")}</span>
                      }
                    </td>
                    <td className="td" style={{ textAlign: "right", color: "#c02020" }}>
                      {t.vendedores?.length ? t.vendedores.map((r,i)=><div key={i} style={{fontSize:10,fontWeight:800}}>{fmt(r.px,4)}</div>) : <span style={{fontSize:10,fontWeight:800}}>{fmt(t.pxVenta,4)}</span>}
                    </td>
